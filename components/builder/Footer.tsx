  ...
  return (
    <footer style={{ borderTop: `1px solid ${B}`, padding: '28px 56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: BG, flexWrap: 'wrap', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
        {linkedinUrl && <a href={linkedinUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(240,234,224,0.2)', textDecoration: 'none' }}>LinkedIn</a>}
        <span style={{ fontSize: 9, color: 'rgba(240,234,224,0.1)' }}>&copy; {new Date().getFullYear()} {companyName}</span>
      </div>
    </footer>
  )
}
