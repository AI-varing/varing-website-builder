    ...
    <section id="contact" style={{ background: '#050505', borderTop: `1px solid ${B}` }}>
      <div style={{ maxWidth: 1300, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
        <div ref={ctaLeft.ref} style={{ padding: '96px 56px', borderRight: `1px solid ${B}`, ...ctaLeft.style }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <a href={`tel:${phone.replace(/[^+\d]/g, '')}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: `1px solid ${B}`, padding: '20px 24px', textDecoration: 'none' }}>
              <div>
                <p style={{ fontSize: 8, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(240,234,224,0.28)', marginBottom: 6 }}>Phone</p>
            ...
            <div style={{ border: `1px solid ${B}`, padding: '20px 24px' }}>
              <p style={{ fontSize: 8, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(240,234,224,0.28)', marginBottom: 6 }}>Office</p>
              <p style={{ fontSize: 13, color: 'rgba(240,234,224,0.55)', lineHeight: 1.5 }}>{officeAddress}</p>
            </div>
          </div>
        </div>
        <div ref={ctaRight.ref} style={{ padding: '96px 56px', ...ctaRight.style }}>
          ...
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.1rem,1.8vw,1.45rem)', fontWeight: 300, fontStyle: 'italic', color: 'rgba(240,234,224,0.14)', lineHeight: 1.7 }}>
            {quote}
          </p>
        </div>
      </div>
