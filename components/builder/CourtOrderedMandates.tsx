        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80 }}>
            ...
            <div style={{ display: 'flex', gap: 28, marginTop: 36, flexWrap: 'wrap' }}>
              {stats.map((s: any, i: number) => (
                <div key={i} style={{ paddingRight: 28, borderRight: i < stats.length - 1 ? `1px solid ${B}` : 'none' }}>
                  ...
                  <p style={{ fontSize: 9, letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(240,234,224,0.28)', marginTop: 6 }}>{s.label}</p>
                </div>
              ))}
            </div>
          </div>
          <div ref={mandRight.ref} style={mandRight.style}>
            <div style={{ borderLeft: `2px solid ${G}`, paddingLeft: 28, marginTop: 4 }}>
              ...
              <p style={{ fontSize: 9, color: 'rgba(240,234,224,0.3)', marginTop: 4, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{quoteRole}</p>
            </div>
          </div>
        </div>
      </div>
