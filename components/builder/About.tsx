...
export default function About({ blok }: { blok?: any }) {
  const heading = blok?.heading || 'Complexity Is\nOur Specialty'
  ...
  const paragraph2 = blok?.paragraph2 || 'Rated <strong style="color:#F0EAE0;fontWeight:700">#1 Agent in BC and Canada</strong> by Homelife International from 2013\u20132024, our team delivers the legal expertise, market intelligence, and network to move complex assets at maximum value.'
  ...
  return (
    <section id="about" style={{ background: BG2, borderTop: `1px solid ${B}`, overflow: 'hidden' }}>
      <div ref={aboutFade.ref} style={{ maxWidth: 1400, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', alignItems: 'stretch', ...aboutFade.style }}>
        <div style={{ padding: '96px 72px 96px 56px', borderRight: `1px solid ${B}` }}>
          <Label>About</Label>
        ...
        <div style={{ position: 'relative', overflow: 'hidden', minHeight: 600, background: '#0a0a0a' }}>
          <div style={{ position: 'absolute', inset: 0, backgroundImage: `radial-gradient(circle, rgba(41,82,163,0.08) 1px, transparent 1px)`, backgroundSize: '32px 32px', pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', fontSize: 'clamp(4rem,10vw,10rem)', fontFamily: "'BentonSans', sans-serif", fontWeight: 900, color: 'rgba(240,234,224,0.03)', letterSpacing: '-0.04em', lineHeight: 1, pointerEvents: 'none', userSelect: 'none', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
            VARING
          </div>
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 2, background: `linear-gradient(to right, transparent, ${G}, transparent)`, opacity: 0.4 }} />
        </div>
      </div>
    </section>
  )
